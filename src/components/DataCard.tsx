...
export default function DataCard({ 
  ...
  return (
    <div className={`group relative bg-gradient-to-br from-[#1A1D29] to-[#161925] rounded-xl p-6 border border-[#2A2D3A]/50 hover:border-[#E33412]/30 transition-all duration-500 hover:shadow-lg hover:shadow-[#E33412]/10 backdrop-blur-sm hover:scale-[1.02] transform animate-fade-in ${className}`}>
      {/* Subtle glow effect */}
      ...
      <div className="relative flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-gray-300 text-sm font-medium group-hover:text-white transition-colors duration-300">{title}</h3>
        </div>
        {change && (
      ...
      <div className="relative mb-2">
        <div className="text-white text-2xl font-bold group-hover:text-[#E33412] transition-colors duration-300 animate-count-up">
          {value}
        </div>
        {subtitle && <div className="text-gray-400 text-sm group-hover:text-gray-300 transition-colors duration-300">{subtitle}</div>}
      </div>
